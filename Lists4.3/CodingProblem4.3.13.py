    ...
    for a_tuple in a_list:
        for value in a_tuple:
            if i == 0:
                ...
                horizontal_component = magnitude * cos(angle_in_radians)
                vertical_component = magnitude * sin(angle_in_radians)
                total_horizontal_force += horizontal_component
                total_vertical_force += vertical_component
                i = 0
    total_magnitude = round(
        sqrt(total_horizontal_force ** 2 + total_vertical_force ** 2), 1)
    new_angle = atan2(total_vertical_force, total_horizontal_force)
    new_angle_in_degrees = round(degrees(new_angle), 1)
    new_tupple = (total_magnitude, new_angle_in_degrees)
    return new_tupple
