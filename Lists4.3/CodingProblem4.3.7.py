...
def wish_list(a_list, string_a, cost, budget):
    for item in a_list:
        if item == string_a:
            if (cost <= budget):
                return ("You should buy a " + str(item) + "!")
            else:
                return ("You should save up for a " + str(item) + "!")
        else:
            continue
    return ("You probably don't want to buy a " + str(string_a) + ".")
