...
def after_second(target_string, search_string):
    length_s_s = len(search_string)
    length_t_s = len(target_string)
    ss_one = target_string.find(search_string)
    ss_two = target_string.find(
        search_string, (ss_one + length_s_s), length_t_s)
    required_string = target_string[(ss_two + length_s_s):]
