...
def string_finder(target_string, search_string):
    ...
    count_search = target_string.find(search_string)
    target_l = target_string[(target_length - 1)]
    search_l = search_string[(search_length - 1)]
    if count_search == 0:
        return "Beginning"
    elif (count_search < (target_length-1) and (count_search > 0)) and (target_l != search_l):
        return "Middle"
    elif (target_l == search_l):
        return "End"
    else:
        return "Not found"
