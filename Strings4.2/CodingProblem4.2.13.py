...
def find_color(string):
    ...
    find_b = string.find(",")
    find_c = string.find(")")
    find_d = string.find(",", find_b+1, find_c)
    red_val = int(string[(find_a+1):find_b])
    green_val = int(string[(find_b+1):find_d])
    blue_val = int(string[(find_d+1):find_c])
    if red_val > green_val and red_val > blue_val:
        return "red"
    elif green_val > red_val and green_val > blue_val:
        return "green"
    elif blue_val > green_val and blue_val > red_val:
        return "blue"
    elif red_val == green_val and red_val != blue_val and green_val != blue_val:
        return "yellow"
    elif red_val == blue_val and red_val != green_val and blue_val != green_val:
        return "purple"
    elif green_val == blue_val and green_val != red_val and blue_val != red_val:
        return "teal"
    else:
        return "gray"
